<div class="cats index">
	...
	<tr<?php echo $class;?>>
		...
		<?php /*<td><?php echo $cat['Cat']['created']; ?>&nbsp;</td>*/?>
		...
		<td class="actions">
			...
			<?php if($cat['Cat']['approved'] == 1){
				echo $this->Html->link(__('Unpublish', true), array('action' => 'publish', $cat['Cat']['id'], 0), null, sprintf(__('Are you sure you want to unpublish # %s?', true), $cat['Cat']['id']));
			}else{
				echo $this->Html->link(__('Publish', true), array('action' => 'publish', $cat['Cat']['id'], 1), null, sprintf(__('Are you sure you want to publish # %s?', true), $cat['Cat']['id']));				
			}?>
		</td>
	</tr>
<?php endforeach; ?>
