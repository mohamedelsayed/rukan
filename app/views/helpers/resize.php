...
class ResizeHelper extends AppHelper{
    ...
    public function smartResizeImage( $file, $width = 960, $height = 0, $proportional = true, $output = 'file'){
        if ( $height <= 0 && $width <= 0 ) {
            return false;
        }        
        if(strpos($file, ROOT) === false){
            $file = IMAGES.'upload'.DS.$file;
        }
        if(is_file($file)){
    ...
    public function check_string_images($str = ''){
        $doc = new DOMDocument();
        @$doc->loadHTML($str);
        $tags = $doc->getElementsByTagName('img');
        foreach ($tags as $tag) {
            $src = $tag->getAttribute('src');
            $this->smartResizeImage(ROOT.$src);
        }        
    }
}